...
export async function POST(
  ...
  await Message.findByIdAndUpdate(id, { $addToSet: { pinnedBy: myId } });
  return corsJson(req, { pinned: true });
}
...
export async function DELETE(
  ...
  const Message = await getMessageModel();
  const myId = new mongoose.Types.ObjectId(userId);
  await Message.findByIdAndUpdate(id, { $pull: { pinnedBy: myId } });
  return corsJson(req, { pinned: false });
}
